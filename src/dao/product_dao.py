...
class ProductDAO:
    ...
    def create_product(self, name: str, sku: str, price: float, stock: int = 0, category: str | None = None) -> Optional[Dict]:
        ...
        self._sb().table("retial_products").insert(payload).execute()
    ...
    def delete_product(self, prod_id: int) -> Optional[Dict]:
        ...
        resp_before = self._sb().table("retial_products").select("*").eq("prod_id", prod_id).limit(1).execute()
        row = resp_before.data[0] if resp_before.data else None
        self._sb().table("retial_products").delete().eq("prod_id", prod_id).execute()
        return row
    
    def list_products(self, limit: int = 100, category: str | None = None) -> List[Dict]:
        q = self._sb().table("retial_products").select("*").order("prod_id", desc=False).limit(limit)
        if category:
